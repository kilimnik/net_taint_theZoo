...
{
    sraSpan *item = (sraSpan*)MemAlloc(sizeof(sraSpan));
    if (item) {
...
{
    if ((IsBadReadPtr(newspan,sizeof(sraSpan))) || (IsBadReadPtr(newspan,sizeof(sraSpan)))) return;
...
{
    if ((IsBadReadPtr(newspan,sizeof(sraSpan))) || (IsBadReadPtr(newspan,sizeof(sraSpan))) || (IsBadReadPtr(before->_prev,sizeof(sraSpan)))) return;
...
{
    sraSpanList *item = (sraSpanList*)MemAlloc(sizeof(sraSpanList));
    if (item) {
...
#define DEFSIZE 4
#define DEFSTEP 8
    i = (sraRectangleIterator*)MemAlloc(sizeof(sraRectangleIterator));
    if(!i)
        return NULL;
    ...
    i->sPtrs = (sraSpan**)MemAlloc(sizeof(sraSpan*)*DEFSIZE);
    if(!i->sPtrs)
    {
        MemFree(i);
        return NULL;
...
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#ifdef SRA_TEST
