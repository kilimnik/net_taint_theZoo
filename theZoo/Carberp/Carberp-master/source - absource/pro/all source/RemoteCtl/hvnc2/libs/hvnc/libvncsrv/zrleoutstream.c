...
{
    buffer->ptr = buffer->start = MemAlloc(size);
...
zrleOutStream *zrleOutStreamNew(void)
{
    ...
    os = MemAlloc(sizeof(zrleOutStream));
    if (os == NULL)
        return NULL;
...
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#ifdef ZRLE_DEBUG
