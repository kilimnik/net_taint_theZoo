...
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#ifdef _HVNC_WEBCAM
        if (lpServer->DeskInfo.bWebCam)
            CamCleanup(lpServer);
        else
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if
        {
            EnterCriticalSection(&lpServer->HotKeysInfo.csHotKeys);
            {
                HOTKEYINFO *lpHotKey=lpServer->HotKeysInfo.lpHotKey,*lpPrev;
                while (lpHotKey)
                {
                    lpPrev=lpHotKey;
            ...
            LeaveCriticalSection(&lpServer->HotKeysInfo.csHotKeys);
            DeleteCriticalSection(&lpServer->HotKeysInfo.csHotKeys);
        }
...
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#ifndef _DEBUG
            break;
#else
            ;
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if
...
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if
        {
            lpServer->HotKeysInfo.hVNC=hVNC;
            InitializeCriticalSection(&lpServer->HotKeysInfo.csHotKeys);
...
static bool IsHotKey(PHVNC lpServer,UINT fsModifiers,UINT dwVk)
{
    bool bRet=false;
    EnterCriticalSection(&lpServer->HotKeysInfo.csHotKeys);
    {
        if (lpServer->HotKeysInfo.lpHotKey)
        {
            HOTKEYINFO *lpHotKey=lpServer->HotKeysInfo.lpHotKey;
            while (lpHotKey)
            {
                if ((lpHotKey->dwVK == dwVk) && (lpHotKey->dwFsModifiers == fsModifiers))
                {
                    bRet=true;
                    break;
                }
...
bool VNCRegisterHotKey(HVNC hVNC,int dwID,UINT dwFsModifiers,UINT dwVK,VNCHOTKEYPROC *lpProc)
{
    bool bRet=false;
    HVNC_HANDLE *lpHandle=VNCGetHandleInformation(hVNC);
    if (lpHandle)
    {
        PHVNC lpServer=lpHandle->lpServer;
        if ((!lpServer->DeskInfo.bWebCam) && (!(dwFsModifiers & ~(MOD_ALT|MOD_CONTROL|MOD_SHIFT|MOD_WIN))) && (!IsHotKey(lpServer,dwFsModifiers,dwVK)))
        {
            EnterCriticalSection(&lpServer->HotKeysInfo.csHotKeys);
            {
                do
                {
                    ...
                    bRet=true;
                    HOTKEYINFO *lpHotKey=(HOTKEYINFO*)MemAlloc(sizeof(HOTKEYINFO));
                    lpHotKey->lpNext=lpServer->HotKeysInfo.lpHotKey;
                    lpHotKey->dwId=dwID;
                    lpHotKey->dwFsModifiers=dwFsModifiers;
                    lpHotKey->dwVK=dwVK;
                    lpHotKey->lpProc=lpProc;
                    lpServer->HotKeysInfo.lpHotKey=lpHotKey;
                }
                while (false);
            }
            LeaveCriticalSection(&lpServer->HotKeysInfo.csHotKeys);
...
bool VNCUnregisterHotKey(HVNC hVNC,int dwID)
{
    bool bRet=false;
    HVNC_HANDLE *lpHandle=VNCGetHandleInformation(hVNC);
    if (lpHandle)
    {
        ...
        if (!lpServer->DeskInfo.bWebCam)
        {
            EnterCriticalSection(&lpServer->HotKeysInfo.csHotKeys);
            {
                HOTKEYINFO *lpHotKey=lpServer->HotKeysInfo.lpHotKey,*lpPrev=NULL;
                while (lpHotKey)
                {
                    if (lpHotKey->dwId == dwID)
                    {
                        if (lpPrev)
                            lpPrev->lpNext=lpHotKey->lpNext;
                        else
                            lpServer->HotKeysInfo.lpHotKey=lpHotKey->lpNext;
                        MemFree(lpHotKey);
                        bRet=true;
                        break;
                    }
