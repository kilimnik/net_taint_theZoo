 ...
 */
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if LIBVNCSERVER_HAVE_LIBPTHREAD && LIBVNCSERVER_HAVE_TLS && !defined(TLS) && defined(__linux__)
#define TLS __thread
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#ifndef TLS
#define TLS
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if
...
void rfbZlibCleanup(rfbScreenInfoPtr screen)
{
    ...
    {
        MemFree(zlibBeforeBuf);
        zlibBeforeBufSize=0;
    }
    if (zlibAfterBufSize)
    {
        zlibAfterBufSize=0;
        MemFree(zlibAfterBuf);
    }
}
                           ...
                           int h)
{
    ...
    if (zlibBeforeBufSize < maxRawSize)
    {
        ...
        if (zlibBeforeBuf == NULL)
            zlibBeforeBuf = (char *)MemAlloc(zlibBeforeBufSize);
        else
            zlibBeforeBuf = (char *)MemRealloc(zlibBeforeBuf, zlibBeforeBufSize);
    }
    ...
    if (zlibAfterBufSize < maxCompSize)
    {
        ...
        if (zlibAfterBuf == NULL)
            zlibAfterBuf = (char *)MemAlloc(zlibAfterBufSize);
        else
            zlibAfterBuf = (char *)MemRealloc(zlibAfterBuf, zlibAfterBufSize);
    }
