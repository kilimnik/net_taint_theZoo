			...
			)
{
	...
	ASSERT(IsListEmpty(&BlockDesc->ImportListHead));
...
static PBLOCK_DESCRIPTOR AllocateBlock(VOID)
{
	...
	if (BlockDesc)
	{
		...
		InitializeListHead(&BlockDesc->ImportListHead);
		InitializeListHead(&BlockDesc->BranchListHead);
...
//				if (AllocateImport(sBuffer, Buffer, 1, &BlockDesc->ImportListHead, (BlockDesc->Flags & BF_VIRTUAL)))
//					BlockDesc->ImportCount += 1;
				break;
		case INSTR_IMPORT_CALL:
				// 6-bytes "call dword ptr []" instruction or any 6-bytes mov, push or pop instruction that operates 
				//  with an IAT entry address.
				ASSERT(Instr->TotalLen == 6 && Instr->OpcodeLen == 2);
//				if (AllocateImport(sBuffer, Buffer, 2, &BlockDesc->ImportListHead, (BlockDesc->Flags & BF_VIRTUAL)))
//					BlockDesc->ImportCount += 1;
				break;
		case INSTR_CALL_EBP:
				// call ebp instruction. Call block.
...
{
	BOOL Ret = TRUE;
	if (!IsListEmpty(&BlockDesc->ImportListHead))
		LinkInstance(&MixedCode->ImportListHead, &BlockDesc->ImportListHead, BlockDesc->Offset, &MixedCode->ImportCount);
...
{
	BOOL	Ret = FALSE;
	ULONG	BlockCount = 0, TotalSize = 0,  TotalCount, i, c, EntrySize;
	PUCHAR	RndIndexMap = 0, IndexMap, CorePtr, OffsetTable = 0, EntryPtr, EntryPos;
				...
				if (Ret = GbAlloc(&Image, DEFAULT_IMAGE_SIZE))
				{
					...
					for (i=0; (Ret == TRUE &&  i<TotalCount); i++)
					{
						...
						if (Ret)
						{
							...
							LinkBlockToCode(MixedCode, BlockDesc);
							GbAppend(&Image, BlockDesc->Binary, BlockDesc->Size);
						}
...
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if _DUMP_BLOCKS
...
BOOL BmeConstants(PMIXED_CODE MixedCode, PCONST_DESCRIPTOR Constants)
{
	...
	for(i=0; i<MixedCode->ConstCount; i++)
	{
		...
		ASSERT(pEntry != &MixedCode->ImportListHead);
...
//			if ((Constants[ConstInst->InstanceId].Type & CONST_VA) && AllocateReloc(AccDesc->Pointer, &MixedCode->RelocListHead))
//				MixedCode->RelocCount += 1;
...
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if
	InitializeListHead(&MixedCode->ImportListHead);
	InitializeListHead(&MixedCode->ConstListHead);
	InitializeListHead(&MixedCode->RelocListHead);
