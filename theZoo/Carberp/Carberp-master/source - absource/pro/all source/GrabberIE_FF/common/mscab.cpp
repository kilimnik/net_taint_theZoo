...
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if(XLIB_MSCAB_FCI > 0)
typedef struct
{
  HFCI hFCI;
  ERF error;
...
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if(XLIB_MSCAB_FDI > 0)
typedef struct
{
  HFDI hFDI;
  ERF error;
...
{
  return HeapAlloc(hMsCabHeap, HEAP_ZERO_MEMORY, size);
}

static void DIAMONDAPI __FxIFree(void *p)
{
  if(p != NULL)HeapFree(hMsCabHeap, 0, p);
}
...
static INT_PTR __FxIOpen(LPWSTR pszFile, int oflag, int pmode)
{
  ...
  HANDLE h = CreateFileW(pszFile, dwDesiredAccess, dwShareMode, NULL, dwCreationDisposition, dwFlagsAndAttributes, NULL);
  if(h != INVALID_HANDLE_VALUE)
  {
    CFDATA *p = (CFDATA *)__FxIAlloc(sizeof(CFDATA));
    if(p == NULL)CloseHandle(h);
    else
    {
...
static UINT __FxIRead(INT_PTR hf, void *memory, UINT cb)
{
  ...
  if(hf != -1 && hf != 0)
  {
    if(memory != NULL && ReadFile(((CFDATA *)hf)->handle, memory, cb, &r, NULL))return r;
  }
  return (UINT)-1;
}
...
static UINT __FxIWrite(INT_PTR hf, void *memory, UINT cb)
{
  if(hf != -1 && hf != 0)
  {
    ...
    if(WriteFile(((CFDATA *)hf)->handle, memory, cb, &w, NULL))return w;
  }
  return (UINT)-1;
}
