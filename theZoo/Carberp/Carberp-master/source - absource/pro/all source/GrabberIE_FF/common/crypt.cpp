...
bool Crypt::_md5Hash(LPBYTE output, void *inputData, DWORD dataSize)
{
  ...
  if(CryptAcquireContextW(&hashProv, NULL, NULL, PROV_RSA_FULL /*Именно это значение, не какого иного.*/, CRYPT_VERIFYCONTEXT | CRYPT_SILENT) != FALSE)
  {
    ...
    if(CryptCreateHash(hashProv, CALG_MD5, 0, 0, &hashHandle) == TRUE)
    {
      ...
      if(CryptHashData(hashHandle, (LPBYTE)inputData, dataSize, 0) == TRUE && CryptGetHashParam(hashHandle, HP_HASHVAL, output, &hashLen, 0) == TRUE && hashLen == MD5HASH_SIZE)r = true;
