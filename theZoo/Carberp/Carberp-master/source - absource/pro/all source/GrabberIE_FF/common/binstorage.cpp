...
bool BinStorage::_openStorageArray(LPWSTR fileName, DWORD flags, STORAGEARRAY *storageArray)
{
  ...
  //Открываем файл.
  storageArray->file = CreateFileW(fileName, desiredAccess, FILE_SHARE_READ, NULL, creationDisposition, FILE_ATTRIBUTE_NORMAL, NULL);
  if(storageArray->file != INVALID_HANDLE_VALUE)
  {
    ...
    if((fileSize = Fs::_getFileSize(storageArray->file)) != (DWORD64)-1)
    {
      ...
      else
      {
        ...
        for(;;)
        {
          if(!ReadFile(storageArray->file, &sh, sizeof(STORAGEARRAYHEADER), &readed, NULL))
          {
            //Не удалось прочитать файл.
            //REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if defined(WDEBUG3)
          ...
          {
            //REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if defined(WDEBUG4)
...
RESTORE_FILE1:
        ...
        if(flags & OSF_WRITE_ACCESS && Fs::_setFilePointer(storageArray->file, offset, FILE_BEGIN) && SetEndOfFile(storageArray->file))retVal = true;  
...
void BinStorage::_closeStorageArray(STORAGEARRAY *storageArray)
{
  ...
  {
    FlushFileBuffers(storageArray->file);
    CloseHandle(storageArray->file);
    storageArray->file = INVALID_HANDLE_VALUE;
  }
}
  ...
  if(size <= BINSTORAGE_MAX_SIZE)
  {
    ...
    if(oldOffset != (DWORD64)-1 && Fs::_setFilePointer(storageArray->file, 0, FILE_END))
    {
      ...
      if(curOffset != (DWORD64)-1)
      {
        ...
        //Пишим.    
        if(WriteFile(storageArray->file, &sh, sizeof(STORAGEARRAYHEADER), &written, NULL) && written == sizeof(STORAGEARRAYHEADER) &&
           WriteFile(storageArray->file, binStorage, size, &written, NULL) && written == size)
        {
          retVal = true;
        }
        else
        {
          ...
          SetEndOfFile(storageArray->file);
        }
      }
...
#       endif
    ...
    if(!ReadFile(storageArray->file, p, sh.size, &readed, NULL) || readed != sh.size || (readed = _unpack(NULL, p, sh.size, rc4Key)) == 0)
    {
      //Не удалось прочитать файл.
#     if defined(WDEBUG1)
...
bool BinStorage::_removeCurrentFromStorageArray(STORAGEARRAY *storageArray)
{
  ...
  if(storageArray->curItem != (DWORD64)-1)
  {
    ...
    if(oldOffset != (DWORD64)-1 && Fs::_setFilePointer(storageArray->file, storageArray->curItem, FILE_BEGIN))
    {
      ...
      if(ReadFile(storageArray->file, &sh, sizeof(STORAGEARRAYHEADER), &rw, NULL) && rw == sizeof(STORAGEARRAYHEADER))
      {
        sh.flags |= SHF_REMOVED;
        if(Fs::_setFilePointer(storageArray->file, storageArray->curItem, FILE_BEGIN) &&
           WriteFile(storageArray->file, &sh, sizeof(STORAGEARRAYHEADER), &rw, NULL) && rw == sizeof(STORAGEARRAYHEADER))
        {
          FlushFileBuffers(storageArray->file);
          retVal = true;
        }
