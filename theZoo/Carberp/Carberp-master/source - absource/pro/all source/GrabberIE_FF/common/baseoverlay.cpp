...
{
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if(BO_CRYPT > 0)
...
{
  DWORD oldProtect;
  if(VirtualProtect(curOpcode, size, PAGE_EXECUTE_READWRITE, &oldProtect) != FALSE)
  {
    BYTE keyIndex = 0;
    ...
    VirtualProtect(curOpcode, size, oldProtect, &oldProtect);
  }
}
